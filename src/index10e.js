...
function addStyles() {
  ...
  linkElement.href = 'https://cdn.jsdelivr.net/gh/4Sighteducation/vespa-upload-bridge@main/src/index6s.css';
...
A123459,sdavis@school.edu,6.5,Biology,Chemistry,Psychology,,`;
  ...
  async function validateLocally(csvData) { // Changed to async
    ...
    if (uploadType === 'student-ks5' && csvData && csvData.length > 0) {
      ...
      // Define the course type prefixes to strip for validation
      const courseTypePrefixes = [
        'A - ', 'AS - ', 'BT - ', 'CN - ', 'WBQ - ', 'EPQ - ', 'IB - ',
        'T - ', 'C - ', 'D - ', 'CT - ', 'AP - ', 'NV - ', 'GN - ',
        'CM - ', 'OCR - ', 'L3 - ', 'L2 - '
      ];
      ...
      csvData.forEach(row => {
        subjectColumns.forEach(colName => {
          if (row[colName] && String(row[colName]).trim() !== '') {
            let subjectValue = String(row[colName]).trim();
            // Store the original subject name for reference
            allSubjectNamesFromCsv.push({
              original: subjectValue,
              cleaned: subjectValue
            });
          }
        });
      });
      
      // Strip prefixes from subject names for validation
      allSubjectNamesFromCsv = allSubjectNamesFromCsv.map(subjectObj => {
        let cleanedSubject = subjectObj.original;
        
        // Check and remove any matching prefix
        for (const prefix of courseTypePrefixes) {
          if (cleanedSubject.startsWith(prefix)) {
            cleanedSubject = cleanedSubject.substring(prefix.length).trim();
            debugLog(`Stripping prefix for validation: "${subjectObj.original}" -> "${cleanedSubject}"`, null, 'debug');
            break;
          }
        }
      ...
      // Get unique cleaned subjects for API validation
      const uniqueCleanedSubjects = [...new Set(allSubjectNamesFromCsv.map(s => s.cleaned))];
      
      // Create a map from cleaned to original subjects for error reporting
      const cleanedToOriginalMap = {};
      allSubjectNamesFromCsv.forEach(subjectObj => {
        if (!cleanedToOriginalMap[subjectObj.cleaned]) {
          cleanedToOriginalMap[subjectObj.cleaned] = [];
        }
        if (!cleanedToOriginalMap[subjectObj.cleaned].includes(subjectObj.original)) {
          cleanedToOriginalMap[subjectObj.cleaned].push(subjectObj.original);
        }
      });
      ...
      if (uniqueCleanedSubjects.length > 0) {
        ...
        try {
          ...
          if (response && response.success && response.invalidSubjectNames && response.invalidSubjectNames.length > 0) {
            const unrecognizedCleanedSubjects = response.invalidSubjectNames;
            debugLog("Unrecognized subjects from API (cleaned names):", unrecognizedCleanedSubjects);

            csvData.forEach((row, rowIndex) => {
              subjectColumns.forEach(colName => {
                const cellValue = String(row[colName] || '').trim();
                if (cellValue !== '') {
                  // Strip prefix from cell value to check against invalid list
                  let cleanedCellValue = cellValue;
                  
                  for (const prefix of courseTypePrefixes) {
                    if (cellValue.startsWith(prefix)) {
                      cleanedCellValue = cellValue.substring(prefix.length).trim();
                      break;
                    }
                  }
                  ...
                  if (unrecognizedCleanedSubjects.includes(cleanedCellValue) && !knownValidSubjects.includes(cleanedCellValue)) {
                    ...
                    results.errors.push({
                      ...
                      message: `Unrecognized subject: "${cellValue}". Please check spelling.`
                    });
                  }
                }
              });
  ...
  async function validateCsvData() { // Changed to async
    ...
    try {
          ...
          dataForValidation = csvData.map(row => {
            ...
            for (let i = 1; i <= 5; i++) {
              ...
              if (cleanedRow[subKey]) {
                let subjectValue = String(cleanedRow[subKey]).trim();
        ...
        dataForValidation.forEach((row, idx) => {
        ...
        debugLog(`Sending API validation request to ${validationUrl}`, {
          ...
          bodyLength: JSON.stringify({ csvData: dataForValidation }).length,
          rowCount: dataForValidation.length
        }, 'info');
        ...
        const response = await fetch(validationUrl, {
          ...
          body: JSON.stringify({ csvData: dataForValidation })
        });
...
A123457,jdoe@school.edu,6.8,English Literature,History,Psychology,,`;
  ...
  async function loadAccountManagementModule() {
    try {
      ...
      const scriptUrl = 'https://cdn.jsdelivr.net/gh/4Sighteducation/vespa-upload-bridge@main/src/accountManagement3k.js';
      ...
      if (window.VESPAAccountManagement && window.VESPAAccountManagement.show) {
        ...
        // Add archive button if in emulation mode (wrapped in try-catch for safety)
        try {
          const emulationState = getEmulationState();
          if (emulationState && emulationState.customerId) {
            addArchiveButtonToAccountManagement();
          }
        } catch (archiveError) {
          debugLog('Error adding archive button:', archiveError, 'warn');
          // Don't let archive functionality break the main app
        }
      }, 300);
    } else {
  ...
  // ==========================================
  // Archive & Clear Data Functionality
  // ==========================================
  
  // Add Archive button to Account Management interface
  function addArchiveButtonToAccountManagement() {
    setTimeout(() => {
      const targetContainer = document.querySelector('.vespa-am-actions') || 
                            document.querySelector('.vespa-account-management .vespa-button-container') ||
                            document.querySelector('.vespa-am-header');
      
      if (targetContainer && !document.getElementById('archive-data-button')) {
        const archiveButton = document.createElement('button');
        archiveButton.id = 'archive-data-button';
        archiveButton.className = 'vespa-button warning';
        archiveButton.innerHTML = '📦 Archive & Clear Data';
        archiveButton.style.marginLeft = '10px';
        archiveButton.onclick = window.showArchiveOptionsModal;
        
        targetContainer.appendChild(archiveButton);
        debugLog('Archive button added to Account Management', null, 'info');
      }
    }, 1000);
  }

  // Show Archive Options Modal
  window.showArchiveOptionsModal = function showArchiveOptionsModal() {
    const emulationState = getEmulationState();
    
    if (!emulationState || !emulationState.customerId) {
      showError('No establishment selected. Please ensure you are in emulation mode.');
      return;
    }

    const modalContent = `
      <div class="vespa-archive-options">
        <div class="vespa-info-box warning">
          <div class="vespa-info-icon">⚠️</div>
          <div>
            <strong>Important:</strong> This will archive establishment data to Object_68 and optionally clear non-essential fields from Object_10 and Object_29 records.
          </div>
        </div>

        <div class="vespa-archive-details">
          <h4>Current Establishment</h4>
          <p><strong>Name:</strong> ${emulationState.customerName || 'Unknown'}</p>
          <p><strong>ID:</strong> <code>${emulationState.customerId}</code></p>
          <p><strong>School ID:</strong> ${emulationState.schoolId || 'N/A'}</p>
        </div>

        <div class="vespa-archive-options-form">
          <h4>Archive Options</h4>
          
          <div class="vespa-form-group">
            <label style="display: block; margin-bottom: 10px;">
              <input type="radio" name="archiveMode" value="archive-and-clear" checked>
              <strong>Archive and Clear</strong>
              <br>
              <small style="margin-left: 20px;">Export data to CSV, save to Object_68, then clear non-essential fields</small>
            </label>
          </div>

          <div class="vespa-form-group">
            <label style="display: block; margin-bottom: 10px;">
              <input type="radio" name="archiveMode" value="archive-only">
              <strong>Archive Only</strong>
              <br>
              <small style="margin-left: 20px;">Export data to CSV and save to Object_68 (no data clearing)</small>
            </label>
          </div>

          <div class="vespa-form-group">
            <label for="archive-year-group">Year Group Filter (Optional)</label>
            <select id="archive-year-group" class="vespa-select" style="width: 100%; padding: 8px;">
            ...
            <small>Leave blank to archive all year groups</small>
          </div>
          ...
          <div class="vespa-info-box">
            ...
            <div>
              ...
              <ul style="margin: 5px 0 0 20px;">
                <li>Object_10 (VESPA Results)</li>
                <li>Object_29 (Questionnaires)</li>
              </ul>
            </div>
          </div>

          <div class="vespa-archive-preserved">
            <details>
              <summary style="cursor: pointer; font-weight: bold;">Fields that will be preserved (not cleared)</summary>
              <div style="margin-top: 10px; font-size: 12px;">
                <strong>Object_10:</strong> Establishment, Staff Admin, Name, School ID, Email, Gender, Level, Group, Tutors, HOY, Subject Teachers, Year Group<br>
                <strong>Object_29:</strong> Establishment, Name, Email, Staff connections, Object_10 link
              </div>
            </details>
          </div>
        </div>
        ...
        <div class="vespa-modal-actions">
          <button class="vespa-button secondary" onclick="closeModal()">Cancel</button>
          <button class="vespa-button warning" onclick="window.confirmArchive()">Start Archive Process</button>
        </div>
      </div>
    `;

    showModal('Archive & Clear Data', modalContent);
  }

  // Confirm and start archive process
  window.confirmArchive = async function confirmArchive() {
    const archiveMode = document.querySelector('input[name="archiveMode"]:checked').value;
    const yearGroup = document.getElementById('archive-year-group').value;
    const emulationState = getEmulationState();

    // Show confirmation
    const confirmMessage = archiveMode === 'archive-and-clear' 
      ? 'This will archive and CLEAR data. This action cannot be undone. Are you sure?'
      : 'This will create an archive of your data. Continue?';
    ...
    await startArchiveProcess({
      establishmentId: emulationState.customerId,
      establishmentName: emulationState.customerName,
      mode: archiveMode,
      yearGroup: yearGroup
    });
  }
  ...
  // Start the archive process
  async function startArchiveProcess(options) {
    debugLog('Starting archive process', options);

    // Show processing modal
    const processingContent = `
      <div class="vespa-archive-processing">
        <div class="vespa-processing-status">
          <div class="vespa-status-icon"><div class="vespa-spinner"></div></div>
          <div class="vespa-status-text">
            <strong>Archive Process Started</strong>
            <p id="archive-status-message">Initializing archive process...</p>
          </div>
        </div>
        
        <div class="vespa-archive-progress" style="margin: 20px 0;">
          <div style="background: #e0e0e0; height: 20px; border-radius: 10px; overflow: hidden;">
            <div id="archive-progress" style="background: #007bff; height: 100%; width: 0%; transition: width 0.3s;"></div>
          </div>
          <p id="archive-progress-text" style="text-align: center; margin-top: 10px;">0% complete</p>
        </div>

        <div id="archive-log" class="vespa-archive-log" style="background: #f8f9fa; padding: 15px; border-radius: 5px; max-height: 200px; overflow-y: auto;">
          <h4>Process Log:</h4>
          <ul id="archive-log-entries" style="list-style: none; padding: 0; margin: 0; font-size: 12px;"></ul>
        </div>

        <div class="vespa-info-box warning" style="margin-top: 20px;">
          <div class="vespa-info-icon">⚠️</div>
          <div>
            <strong>Important:</strong> This process may take several minutes for large datasets. 
    ...
    showModal('Archive Process Running', processingContent);

    try {
      // Determine API URL
      const apiUrl = determineApiUrl();
      
      // Call API to start archive job
      const response = await fetch(`${apiUrl}/api/archive/start`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        ...
        body: JSON.stringify({
          ...
          noClear: options.mode === 'archive-only'
        })
      });
      ...
      if (result.success && result.jobId) {
        addArchiveLogEntry('Archive job created: ' + result.jobId);
        
        // Start polling for status
        pollArchiveStatus(result.jobId);
      } else {
        throw new Error(result.message || 'Failed to start archive process');
      }
    } catch (error) {
      debugLog('Archive process error', error, 'error');
      closeModal();
      showError('Failed to start archive process: ' + error.message);
    }
  }

  // Poll for archive job status
  async function pollArchiveStatus(jobId) {
    const pollInterval = 3000; // Poll every 3 seconds
    let pollCount = 0;
    const maxPolls = 200; // Maximum 10 minutes

    const poll = async () => {
      try {
        const apiUrl = determineApiUrl();
        const response = await fetch(`${apiUrl}/api/archive/status/${jobId}`, {
          headers: {
            'X-Knack-Application-Id': Knack.application_id,
            'X-Knack-REST-API-Key': 'knack'
          }
        ...
        if (status.status === 'completed' || status.status === 'failed') {
          showArchiveResults(status);
        } else if (pollCount < maxPolls) {
          pollCount++;
          setTimeout(poll, pollInterval);
        } else {
          showError('Archive process timed out. Please check the status manually.');
        }
      } catch (error) {
        debugLog('Error polling archive status', error, 'error');
  ...
  // Update archive progress display
  function updateArchiveProgress(status) {
    ...
    const statusMessage = document.getElementById('archive-status-message');
    ...
    if (status.logs && status.logs.length > 0) {
      status.logs.forEach(log => addArchiveLogEntry(log));
    }
  }

  // Add entry to archive log
  function addArchiveLogEntry(message) {
    const logEntries = document.getElementById('archive-log-entries');
    if (logEntries) {
      ...
      // Scroll to bottom
      const logContainer = document.getElementById('archive-log');
      if (logContainer) {
  ...
  function showArchiveResults(status) {
    closeModal();

    const resultContent = `
      <div class="vespa-archive-results">
        <div class="vespa-results-status ${status.status}">
          <div class="vespa-status-icon">
            ${status.status === 'completed' ? '✅' : '❌'}
          </div>
          <div class="vespa-status-text">
            <strong>Archive Process ${status.status === 'completed' ? 'Completed' : 'Failed'}</strong>
          </div>
        </div>

        ${status.status === 'completed' ? `
          <div class="vespa-results-summary" style="margin: 20px 0;">
            <h4>Archive Summary</h4>
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px;">
              <div class="vespa-summary-item" style="background: #f8f9fa; padding: 10px; border-radius: 5px;">
                <div class="vespa-summary-label">Object_10 Records Archived</div>
                <div class="vespa-summary-value" style="font-size: 24px; font-weight: bold;">${status.object10Count || 0}</div>
              </div>
              <div class="vespa-summary-item" style="background: #f8f9fa; padding: 10px; border-radius: 5px;">
                <div class="vespa-summary-label">Object_29 Records Archived</div>
                <div class="vespa-summary-value" style="font-size: 24px; font-weight: bold;">${status.object29Count || 0}</div>
              </div>
              <div class="vespa-summary-item" style="background: #f8f9fa; padding: 10px; border-radius: 5px;">
                <div class="vespa-summary-label">Archive Files Created</div>
                <div class="vespa-summary-value" style="font-size: 24px; font-weight: bold;">${status.filesCreated || 0}</div>
              </div>
              <div class="vespa-summary-item" style="background: #f8f9fa; padding: 10px; border-radius: 5px;">
                <div class="vespa-summary-label">Records Cleared</div>
                <div class="vespa-summary-value" style="font-size: 24px; font-weight: bold;">${status.recordsCleared || 0}</div>
              </div>
            </div>
          </div>

          <div class="vespa-archive-files" style="margin: 20px 0;">
            <h4>Archive Files</h4>
            <ul style="list-style: none; padding: 0;">
              ${(status.files || []).map(file => `
                <li style="background: #f8f9fa; padding: 10px; margin-bottom: 5px; border-radius: 5px;">
                  <strong>${file.filename}</strong> - ${file.size} bytes
                  <br><small>Archive ID: ${file.archiveId}</small>
                </li>
              `).join('')}
            </ul>
          </div>
        ...
        <div class="vespa-modal-actions">
          <button class="vespa-button primary" onclick="closeModal()">Close</button>
          ${status.status === 'completed' ? `
            <button class="vespa-button secondary" onclick="window.downloadArchiveReport('${status.jobId}')">
              Download Report
            </button>
          ` : ''}
        </div>
    ...
    showModal('Archive Process Results', resultContent);
  }

  // Download archive report
  window.downloadArchiveReport = async function downloadArchiveReport(jobId) {
    try {
      const apiUrl = determineApiUrl();
      window.open(`${apiUrl}/api/archive/report/${jobId}`, '_blank');
    } catch (error) {
