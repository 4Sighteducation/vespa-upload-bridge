          ...
          getUploaderContext() {
            ...
            if (isEmulating) {
              ...
              return {
                ...
                userEmail: this.userEmail,
                isEmulating: false,
                customerId: this.schoolContext?.knackCustomerId
              };
            }
          },
          ...
          async loadManualAddOptions() {
            try {
              const customerId = this.isSuperUser && this.selectedSchool?.knackId
                ...
                : this.schoolContext?.knackCustomerId;
              
              if (!customerId) return;
              
              const response = await fetch(
                `${this.apiUrl}/api/validation/get-form-options?customerId=${customerId}`,
                {
                  method: 'GET',
                  headers: { 'Content-Type': 'application/json' }
                }
              ...
              if (data.success && data.options) {
                this.availableStaff = {
                  tutors: data.options.tutors || [],
                  headsOfYear: data.options.headsOfYear || [],
                  subjectTeachers: data.options.subjectTeachers || [],
                  staffAdmins: []
                };
                this.availableGroups = data.options.groups || [];
                debugLog('Manual add options loaded', data.options);
              }
        ...
        template: `
          <div class="vespa-account-manager">
            ...
            <div class="am-toolbar">
              <div class="am-toolbar-left">
                ...
                <div v-if="!hasSelectedAccounts && isSuperUser && !selectedSchool" style="padding: 10px 16px; background: #fff3cd; border-radius: 8px; color: #856404; font-size: 14px; font-weight: 500;">
                  ℹ️ Select a school to upload data or manage groups
                </div>
            ...
            <div v-if="showManualAddModal" class="am-modal-overlay" @click.self="closeManualAddModal">
              <div class="am-modal">
                <div class="am-modal-header">
                  <h3>➕ Add {{ manualAddType === 'students' ? 'Student' : 'Staff Member' }}</h3>
                  <button @click="closeManualAddModal" class="am-modal-close">✖</button>
                </div>
                
                <div class="am-modal-body">
                  <!-- Staff Form -->
                  <div v-if="manualAddType === 'staff'">
                    <div style="display: grid; grid-template-columns: auto 1fr 1fr; gap: 12px; margin-bottom: 16px;">
                      <div>
                        <label style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 13px;">Title *</label>
                        <input v-model="manualAddForm.title" placeholder="Mr/Ms/Dr" class="am-input-inline" required />
                      </div>
                      <div>
                        <label style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 13px;">First Name *</label>
                        <input v-model="manualAddForm.firstName" class="am-input-inline" required />
                      </div>
                      <div>
                        <label style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 13px;">Last Name *</label>
                        <input v-model="manualAddForm.lastName" class="am-input-inline" required />
                      </div>
                    </div>
                    
                    <div style="margin-bottom: 16px;">
                      <label style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 13px;">Email *</label>
                      <input v-model="manualAddForm.email" type="email" class="am-input-inline" required />
                    </div>
                    
                    <div style="margin-bottom: 16px;">
                      <label style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 13px;">Staff Type(s) *</label>
                      <select v-model="manualAddForm.staffTypes" multiple class="am-select-inline" style="height: 120px;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">
                      <div>
                        <label style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 13px;">Year Group</label>
                        <select v-model="manualAddForm.yearGroup" class="am-select-inline">
                          <option value="">-- Optional --</option>
                          <option v-for="year in yearGroups" :key="year" :value="year">Year {{ year }}</option>
                        </select>
                      </div>
                      <div>
                        <label style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 13px;">Tutor Group</label>
                        <select v-model="manualAddForm.group" class="am-select-inline">
                          <option value="">-- Optional --</option>
                    ...
                    <div style="margin-bottom: 16px;">
                      ...
                      <input v-model="manualAddForm.subject" placeholder="e.g., Physics" class="am-input-inline" />
                    </div>
                  </div>
                  
                  <!-- Student Form -->
                  <div v-if="manualAddType === 'students'">
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">
                      <div>
                        <label style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 13px;">First Name *</label>
                        <input v-model="manualAddForm.firstName" class="am-input-inline" required />
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">
                      ...
                      <div>
                        <label style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 13px;">UPN</label>
                        <input v-model="manualAddForm.upn" placeholder="Auto-generated if blank" class="am-input-inline" />
                      </div>
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 12px; margin-bottom: 16px;">
                      <div>
                        <label style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 13px;">Year Group *</label>
                        <select v-model="manualAddForm.yearGroup" class="am-select-inline" required>
                          <option value="">-- Select --</option>
                          <option v-for="year in yearGroups" :key="year" :value="year">Year {{ year }}</option>
                        </select>
                      </div>
                      <div>
                        <label style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 13px;">Level *</label>
                        <select v-model="manualAddForm.level" class="am-select-inline" required>
                          <option value="">-- Select --</option>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">
                      <div>
                        <label style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 13px;">Gender</label>
                        <select v-model="manualAddForm.gender" class="am-select-inline">
                          <option value="">-- Optional --</option>
                          <option value="Male">Male</option>
                          <option value="Female">Female</option>
                          <option value="Non-Binary">Non-Binary</option>
                      ...
                      <div>
                        <label style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 13px;">Date of Birth</label>
                        <input v-model="manualAddForm.dob" type="date" class="am-input-inline" />
                      </div>
                    </div>
                    
                    <div style="margin-bottom: 16px;">
                      <label style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 13px;">Tutors</label>
                      <select v-model="manualAddForm.tutors" multiple class="am-select-inline" style="height: 100px;">
                    ...
                    <div style="margin-bottom: 16px;">
                      <label style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 13px;">Head of Year</label>
                      <select v-model="manualAddForm.headOfYear" class="am-select-inline">
                        <option value="">-- Optional --</option>
                    ...
                    <div style="margin-bottom: 16px;">
                      <label style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 13px;">Subject Teachers</label>
                      <select v-model="manualAddForm.subjectTeachers" multiple class="am-select-inline" style="height: 100px;">
                  ...
                  <div style="margin-top: 20px; padding: 12px; background: #fff3cd; border-radius: 6px; font-size: 13px;">
                    ℹ️ A temporary password will be auto-generated and sent via email
                  </div>
                </div>
                
                <div class="am-modal-footer">
                  <button @click="closeManualAddModal" class="am-button secondary">
                    Cancel
                  </button>
                  <button 
