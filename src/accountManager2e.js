      ...
      const app = createApp({
        ...
        methods: {
          ...
          async loadKnackSchoolsForSync() {
            this.loading = true;
            this.loadingText = 'Loading all establishments from Knack...';
            
            try {
              // Call backend to get ALL Knack schools + their Supabase status
              const response = await fetch(
                `${this.apiUrl}/api/v3/establishments/status`,
                {
                  method: 'GET',
                  headers: { 'Content-Type': 'application/json' }
                }
              ...
              if (data.success && data.schools) {
                this.knackSchools = data.schools;
                
                // Build status map
                this.schoolSyncStatus = {};
                data.schools.forEach(school => {
                  this.schoolSyncStatus[school.knackId] = {
                    inSupabase: school.inSupabase || false,
                    supabaseUuid: school.supabaseUuid || null,
                    syncing: false,
                    accounts: school.accounts || { staff: 0, students: 0 }
                  };
                });
                
                debugLog('Loaded Knack schools for sync', { 
              ...
              console.error('Load Knack schools error:', error);
